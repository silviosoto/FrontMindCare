import { get, post, del, put, putcustom } from "./apiService";
...
export const getPaciente = async( id )  => {
    try {
        const  data  = await  get( `paciente/${id}`);
        const result = data ? data : [];
        return result
    }
    catch (ex) {
...
export const updatePacient = async ( id, paciente) => {
    try {
        return  await put( `paciente/${id}`, paciente); 
    }
    catch (ex) {
        rej(ex);
    }
...
export const updateImagePacient = async ( id, paciente) => {
    try {

        return  await putcustom( `paciente/image/${id}`, paciente); 
    }
    catch (ex) {
        rej(ex);
    }
